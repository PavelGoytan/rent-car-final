...
<div class="container">
    ...
    <c:if test="${requestScope.modelName!=null}">
        <div>
        <span>
            ${requestScope.modelName} ${requestScope.modelNumber} is not available!
        </span>
        </div>
    </c:if>
            ...
            <tr>
                ...
                <td>
                    <div class="btn-group" role="group" aria-label="Basic example">
                        ...
                        car.carStatus==sessionScope.impossible}">
                                <a class="nav-link disabled">
                                    <button type="button" class="btn btn-primary" disabled>ORDERED</button>
                                </a>
                                <c:if test="${sessionScope.isAdmin==true||sessionScope.isManager==true}">
                            ...
                            <c:otherwise>
                                <a href="${pageContext.request.contextPath}/controller?command=go_to_create_order&cost=${car.cost}&id=${car.id}">
                                    <button class="btn btn-primary">ORDERED</button>
                                </a>
                                <c:if test="${sessionScope.isAdmin==true||sessionScope.isManager==true}">
                                    <a href="${pageContext.request.contextPath}/controller?command=go_to_update_car&id=${car.id}">
                                        <button class="btn btn-primary">UPDATE</button>
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination">
            ...
            <c:if test="${sessionScope.page_number>1}">
                <a class="page-link"
                   href="${pageContext.request.contextPath}/controller?command=go_to_home_page&page_number=${previous}">&lt;&lt;previous</a>
            </c:if>
            <c:if test="${sessionScope.max_page>1}">
                <c:forEach var="pages" items="${sessionScope.pages}">
                    <a class="page-link"
                       href="${pageContext.request.contextPath}/controller?command=go_to_home_page&page_number=${pages}">${pages}</a>
                </c:forEach>
            </c:if>

            <c:if test="${sessionScope.page_number<sessionScope.max_page}">
                <a class="page-link"
                   href="${pageContext.request.contextPath}/controller?command=go_to_home_page&page_number=${next}">next&gt;&gt;</a>
            </c:if>
        </ul>
    </nav>
    <br>
